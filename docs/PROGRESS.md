...
> Last updated: 2026-05-05 by claude (Phase 3.2 complete, beginning 3.3)
> Build phase: 3 (vertical slice — Tasks 3.1, 3.2 done)
> v1 ETA: 2026-07-15
...
- Phase 3 vertical slice: 🚧 (3.1 ✅, 3.2 ✅, 3.3 next)
...
1. **READY** — Phase 3 Task 3.3: TrainSystem v0 — first vector-rendered Engine car (left-anchored at `x≈200` per ADR-001 §Gap 2), parallax background scrolling at `worldVelocity = 50 px/sec`. Implements: TrainSystem class, cars.json with Engine entry, JSON shape recipe rendering (per ADR-001 §Gap 3), `drawRecipe()` helper.
2. **GATED ON 3.3** — Phase 3 Task 3.4: ModuleAttachmentSystem v0 (typed slots per ADR-001 §Gap 1, tagged behavior registry per §Gap 6) — *most important task in Phase 3*; reviewer pass mandatory before merge.
3. **GATED ON 3.4** — Phase 3 Task 3.5: CombatSystem + EnemySpawner v0 (cannon auto-fires at scout, +1 Salvage per kill).
4. **GATED ON 3.5** — Phase 3 Task 3.6: SaveSystem v0 (saveVersion: 1, totalSalvage persists, migration runner stub).
5. **GATED ON 3.6** — Phase 3 Task 3.7: Reviewer/Opus end-of-phase audit.
...
- Total source: 76 LoC (`src/**/*.ts`) after Task 3.2
- Test coverage: 4 unit tests passing (gameConfig shape + scene order)
